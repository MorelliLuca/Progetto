...
void Print(std::vector<Population> const& simulated)  // Funzione che stampa la tabella con i risultati
{
  ...
  for (auto it = simulated.begin(); it != simulated.end(); ++it) {
    ...
    {
      std::cout << "| " << std::setw(Term_width_day) << std::distance(simulated.begin(), it) << " | " << std::setw(Term_width_SIR) << it->S() << " | " << std::setw(Term_width_SIR)
                << it->I() << " | " << std::setw(Term_width_SIR) << it->R() << " | " << std::setw(Term_width_beta) << it->Beta() << " | " << std::setw(Term_width_gamma)
                << it->Gamma() << " | " << std::setw(Term_width_q) << it->Quarantine() << " | " << std::setw(Term_width_n_vax) << it->N_vax() << " |\n";
    } else {
      if (it->S() >= it->N_vax())  // Stampa della tabella con il numero di vaccinati totali se S è maggiore del numero di vaccini giornalieri
      {
        ...
        std::cout << "| " << std::setw(Term_width_day) << std::distance(simulated.begin(), it) << " | " << std::setw(Term_width_SIR) << it->S() << " | "
                  << std::setw(Term_width_SIR) << it->I() << " | " << std::setw(Term_width_SIR) << it->R() << " | " << std::setw(Term_width_beta) << it->Beta() << " | "
                  << std::setw(Term_width_gamma) << it->Gamma() << " | " << std::setw(Term_width_q) << it->Quarantine() << " | " << std::setw(Term_width_n_vax) << (it->N_vax() * i)
                  << " |\n";
      } else {
        if (it->S() != Data::min)  // Stampa della tabella con i vaccinati totali aumentati di S e non del numero di vaccini giornalieri
        {
          std::cout << "| " << std::setw(Term_width_day) << std::distance(simulated.begin(), it) << " | " << std::setw(Term_width_SIR) << it->S() << " | "
                    << std::setw(Term_width_SIR) << it->I() << " | " << std::setw(Term_width_SIR) << it->R() << " | " << std::setw(Term_width_beta) << it->Beta() << " | "
                    << std::setw(Term_width_gamma) << it->Gamma() << " | " << std::setw(Term_width_q) << it->Quarantine() << " | " << std::setw(Term_width_n_vax)
                    << ((it->N_vax() * i) + (it)->S()) << " |\n";
