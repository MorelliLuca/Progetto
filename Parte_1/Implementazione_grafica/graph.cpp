  ...
  for (auto it = data_vector.begin(); it != data_vector.end(); ++it) {
    if (it != data_vector.begin() && it->I() <= (it - 1)->I())  // Dimensioni della colonna se il dato precedente era maggiore o uguale
    {
      line.setSize(sf::Vector2f(
          epsilon_x,
          Correction_size + epsilon_y * ((it - 1)->I() - it->I())));  // l'altezza è data da 3 pixel più la differenza tra gli infetti, in modo da ridurre gli spazi bianchi
    } else {
...
void print_axis(sf::RenderWindow& window, std::vector<Simulation::Population> const& data_vector)  // Stampa degli assi del grafico
{
  ...
  for (int i{1}; i < Axis_division; ++i) {
    ...
    label.setPosition(
        sf::Vector2f(Space_caption + (i * (window.getSize().x - Space_caption) / Axis_division - Font_dimension / 2), window.getSize().y - Line_length - Font_dimension));
    window.draw(label);
  }
