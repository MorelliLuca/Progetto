...
void print_axis(sf::RenderWindow& window, std::vector<Simulation::Population> const& data_vector) //Stampa degli assi del grafico
{
  ...
  sf::Font font; 
  if (!font.loadFromFile("font.ttf")) { throw std::runtime_error{"Font was not loaded correctly"}; } //Controllo sul corretto caricamento del font
  ...
  window.draw(label);
  line.rotate(90); //Rotazione di 90° necessaria per stmpare la scala sull'asse x
